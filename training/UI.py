...
def test_one_dataset(model, data_loader):
    ...
    for i, data_dict in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        predicted_labels = torch.where(prob > 0.5, 1, 0)
...
def load_model(weights_path):
    # Giả sử model của bạn là một phần của module 'DETECTOR'
...
def main():
    ...
    if on_2060:
        ...
        config['workers'] = 8
        config['lmdb_dir'] = r'./datasets/LMDBs'
    weights_path = None
    ...
    model = load_model('E:/TLCN/Main/training/weights/capsule_net_best.pth')
